...
def datetime_to_jd(t: datetime) -> Tuple[float, float]:
    ...
    jd1 = jd1 + jd2 - 0.5
    frac = (t.hour * 3600.0 + t.minute * 60.0 + t.second + t.microsecond / 1.0E6) / 86400.0 + 0.5
    if (frac >= 1.0):
        jd1 += 1.0
        frac -= 1.0
    return jd1, frac
...
def isoformat_to_jd(utc: str) -> Tuple[float, float]:
    ...
    x = datetime_to_jd(t)                                                                 # and from datetime to mjd
    return x
...
def _iterate_over_utc_array(utc, elementfunc):
    ...
    jd1 = np.zeros(s, dtype=np.float64)                                                                                     # create the output array for the high order julian date components
    jd2 = np.zeros(s, dtype=np.float64)                                                                                     # create the output array for the low order julian date component
    with np.nditer([utc, jd1, jd2], flags=['refs_ok'], op_flags=[['readonly'], ['writeonly'], ['writeonly']]) as it:     # create the numpy iterator object
        for a, b, c in it:                                                                                                  # and convert each elemnet
            j1, j2 = elementfunc(a.item())                                                                                  # from the numpy array element datetime to jd
            b[...] = j1
            c[...] = j2
        jd1 = it.operands[1]
        jd2 = it.operands[2]
...
#           utc_to_jd
# ------------------------------------------------------------------------------
def ut_to_jd(ut: Union[np.ndarray, List[Any], Any]):
    """
    ...
    if type(ut) is np.ndarray:                                                                                 # Handle an numpy array of values, datetime64, datetime objects or mjd numbers
        if ut.dtype.kind == 'M':                                                                                # if we have a numpy array of datetime64 object
            jd1, jd2 = _iterate_over_utc_array(ut, datetime64_to_jd)                                         # Then convert all the elements to julian date
        elif (ut.dtype.kind == 'O') and (len(ut) > 0) and (type(ut[0]) is datetime):                           # otherwise if we have a numpy array of  datetime objects
            jd1, jd2 = _iterate_over_utc_array(ut, datetime_to_jd)                                           # create the output array
        elif (ut.dtype.kind == 'U'):
            jd1, jd2 = _iterate_over_utc_array(ut, isoformat_to_jd)                                          # otherwise if the array is strings
        elif (ut.dtype.kind == 'f') or ((ut.dtype.kind == 'i')):
            jd1, jd2 = _iterate_over_utc_array(ut, number_to_jd)                                               # otherwise if the array is strings
    elif type(ut) is str:                                                                                      # if we have a single string
        ...
        if (type(ut) is datetime):                                                                             # if its a datetime object
            ...
            jd1 = math.nan
            jd2 = math.nan
            raise Exception('utc_to_jd, unsupported data type {}. Cannot convert to julian date'.format(str(type(utc))))
    return jd1, jd2
